...
- `personaNotes`, `toneNotes`, `activityNotes`는 관리자 화면에서 생성/수정/조회할 수 있습니다.
- 현재는 **ADMIN 전용 자동 게시글/댓글 초안 생성**에서 프롬프트 컨텍스트로 사용됩니다.
- 자동 생성은 추천/초안 채우기만 수행하며 자동 게시는 하지 않습니다.
...
- 관리자 입력/수정/조회 가능
- ADMIN 전용 자동 생성(게시글/댓글) 프롬프트 컨텍스트로 사용
- 운영계정별 톤/활동 가이드 반영
- 자동 생성 결과는 게시 전 수동 검토가 필요
